...
@MainActor enum NativeFullLabelProbe {
    /// Full-label buttons have no explicit accessibility name override, so
    /// SwiftUI derives their name from the rendered title text.
    static func check(store: Store, panel: PanelAdapter, navigationFrame: NSRect?) throws -> NSRect {
        ...
        let titles = Set(store.view.items.map(\.title))
        let buttons = NativeAccessibility.elements(in: content).filter {
            $0.role == .button && $0.label.map(titles.contains) == true
        }
        guard buttons.count == store.view.items.count else { throw ProbeFailure("Missing native full-title buttons") }
        for (item, label) in zip(store.view.items, layout.labels) {
            ...
            guard buttons.contains(where: { button in
                guard button.label == item.title, let frame = button.frame else { return false }
